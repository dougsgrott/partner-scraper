"""Live smoke test for issue 03 — fetch+enrich on a handful of real URLs.

Run:  set -a; [ -f .env ] && . ./.env; set +a; uv run python scripts/smoke_ingest.py
Needs ANTHROPIC_API_KEY in the environment (or a .env file, which is git-ignored).
"""
...
from scraper.ingest.fetch_enrich import fetch_enrich
